...
fn bench_cache_set_get(c: &mut Criterion) {
    ...
    group.bench_function("set_1000_keys", |b| {
        b.iter(|| {
            for i in 0..1000 {
                shim.set(&format!("key_{}", i), b"value_data".to_vec(), 3600);
            }
        });
    });
...
fn bench_encryption(c: &mut Criterion) {
    ...
    let shim = EncryptionShim::new_with_key(key);

    let mut group = c.benchmark_group("encryption_throughput");
    group.bench_function("aes_gcm_encrypt_4kb", |b| {
        b.iter(|| shim.encrypt_aes_gcm(&data).unwrap());
    });

    let encrypted = shim.encrypt_aes_gcm(&data).unwrap();
    group.bench_function("aes_gcm_decrypt_4kb", |b| {
        b.iter(|| shim.decrypt_aes_gcm(&encrypted).unwrap());
    });
